...
The two usage bars read straight from the JSON payload **Claude Code sends the status line on stdin** (`rate_limits.five_hour.used_percentage` / `rate_limits.seven_day.used_percentage`, plus `five_hour.resets_at`). These are the authoritative server-side numbers covering *all* your usage (web + Code) — the same ones `/usage` shows. No cache file, no background worker, no cost estimation, no calibration.

If Claude Code doesn't send `rate_limits` (older builds), the status line falls back to an estimate from [`ccusage`](https://github.com/ryoppippi/ccusage) (`5-hour cost ÷ cap`). That path is approximate — see calibration below.
...
Removes the `statusLine` key from `settings.json` and deletes the status-line scripts plus their caches and locks (including `widget_refresh.ps1` and `_widget.lock` left over from older installs). Leaves `widget_limits.json` and `effortLevel` untouched.
...
| `$USAGE_CAP_USD` | **Fallback only.** The cost-equivalent 5-hour cap. Only matters on Claude Code builds that don't send `rate_limits`. Calibrate to `current_5h_cost / (usage_percent / 100)`. |
...
- **`statusline_model.ps1`** renders the line: model, effort, usage and reset time all from the stdin payload (fallback `_usage5h.json`), context size from the transcript. It spawns nothing and does no I/O beyond reading the transcript.
